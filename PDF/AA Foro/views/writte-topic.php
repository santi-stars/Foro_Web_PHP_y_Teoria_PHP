...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($topic_name != "" && $comment_text != "") {
        ...
        $user_id = UserController::get_user_id_by_user_nick($_SESSION['user']);
        // registramos el nuevo tema con su categoria y el usuario que lo registra
        require_once '..\controllers\topics_controller.php';
        TopicsController::register_topic($topic_name, $user_id->user_id, $cat_id);
        // recuperamos el último id para ingresar el primer mensaje del tema a la vez que registra el tema
        $topic_id = TopicsController::get_last_topic_id();
        require_once '..\controllers\comments_controller.php';
        CommentsController::register_comment($comment_text, $user_id->user_id, $topic_id->last_id);
        header("location: writte-topic.php?sessionExists=" . $_GET['sessionExists'] . "&cat_id". $cat_id);
    }
}
            ...
            <div id="content">
                <div class="notice">
                    <form class="login-form"
                        ...
                        <!--    <input class="hidden" value="<?php echo $_GET['sessionExists'] ?>"><br>
                        <input class="hidden" value="<?php echo $_GET['cat_id'] ?>"><br> -->
